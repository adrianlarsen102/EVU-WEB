...
// Lazy-load the client to prevent Next.js build errors when env vars are missing
...
async function verifyDatabaseSchema() {
  ...
  if (missingTables.length > 0) {
    ...
    console.error('The following essential tables are missing from your database:');
    missingTables.forEach(t => console.error(` - ${t}`));
    console.error('\nPlease run the appropriate migration scripts in your Supabase dashboard:');
    console.error(' - docs/database/complete-migration-2025.sql');
    console.error(' - docs/database/rbac-setup.sql');
    console.error(' - docs/database/audit-logs.sql');
    console.error(' - docs/database/metrics-history.sql');
    console.error(' - docs/database/complete-discord-setup.sql');
    console.error(' - docs/database/error-logs.sql');
    console.error('='.repeat(60) + '\n');
    return false;
  }
...
async function initializeDatabase() {
  ...
  try {
    ...
    if (!isSchemaValid) {
      // Bail out safely instead of throwing unhandled errors later
      console.error('Initialization aborted due to missing tables.');
      return;
    }
    ...
    // We no longer auto-create an admin here. 
    // This is handled by the /setup page.
 ...
 */
async function setupAdmin(username, displayName, password) {
  try {
...
async function verifyPassword(username, password) {
  ...
  try {
    const { data: admins, error } = await supabase
      .from('admins')
      .select('id, username, password_hash, is_default_password, role, is_admin')
...
// But skip auto-initialization during tests and build time to prevent detached async errors
...
async function getTopicsByCategory(categoryId, options = {}) {
  await initializeDatabase();

  try {
    // Pagination options with defaults
    const limit = options.limit || 20; // Default 20 topics per page
    const offset = options.offset || 0;

    // Build query with pagination
    let query = supabase
      .from('forum_topics')
      .select('*, comment_count:forum_comments(count)', { count: 'exact' })
...
async function updateTopicViewCount(topicId) {
  ...
  try {
    ...
    // If RPC doesn't exist, use regular update fallback
    try {
...
async function incrementCategoryTopicCount(categoryId) {
  ...
  try {
    // RACE CONDITION FIX: Use PostgreSQL's jsonb functions for atomic update
    // This prevents lost updates when multiple requests happen concurrently

    // First, get the array index of the category
    ...
    if (!contentData?.content?.forumCategories) {
      return { success: false, error: 'No forum categories found' };
    }

    const categoryIndex = contentData.content.forumCategories.findIndex(
    ...
    // Use raw SQL with jsonb_set for atomic increment
    ...
    if (error) {
      ...
      const categories = [...contentData.content.forumCategories];
      categories[categoryIndex] = {
        ...categories[categoryIndex],
        topics: (categories[categoryIndex].topics || 0) + 1
      };
...
async function incrementCategoryPostCount(categoryId) {
  ...
  try {
    // RACE CONDITION FIX: Use atomic update like incrementCategoryTopicCount
    ...
    // Use raw SQL with jsonb_set for atomic increment
    const { error } = await supabase.rpc('increment_category_posts', {
      category_index: categoryIndex
    });
    ...
    if (error) {
      ...
      categories[categoryIndex] = {
        ...categories[categoryIndex],
        posts: (categories[categoryIndex].posts || 0) + 1
      };
...
async function getCommentsByTopic(topicId, options = {}) {
  await initializeDatabase();

  try {
    // Pagination options with defaults
    const limit = options.limit || 50; // Default 50 comments per page
    const offset = options.offset || 0;
