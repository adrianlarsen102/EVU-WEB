  ...
  try {
    ...
    if (isValid) {
      return {
        ...
        isAdmin: admin.role === 'admin' || admin.is_admin === true
      };
    }
  ...
  try {
    ...
    if (session && session.admins) {
      return {
        ...
        role: session.admins.role || 'user',
        isAdmin: session.admins.role === 'admin' || session.admins.is_admin === true
      };
    }
...
async function createAdmin(username, password, isDefaultPassword = true, role = 'user') {
  await initializeDatabase();
