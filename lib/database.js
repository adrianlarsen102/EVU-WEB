...
async function initializeDatabase() {
  ...
  try {
    ...
    if (!admins || admins.length === 0) {
      // SECURITY: Generate cryptographically random password instead of hardcoded
      ...
      if (insertError) {
        ...
        // SECURITY: Log password to secure location only (not plaintext in logs)
        // In production, this should be sent via secure channel or written to a secure file
        console.log('✅ Default admin created. Username: admin');
        console.log('⚠️  IMPORTANT: Initial password stored in ADMIN_INITIAL_PASSWORD env var or check secure logs');
        // Only log password in development mode for initial setup
        if (process.env.NODE_ENV === 'development') {
          console.log(`🔐 DEV ONLY - Initial admin password: ${randomPassword}`);
        }
      }
    }
