...
import { logError, ErrorTypes as ErrorLogTypes, ErrorSeverity, extractErrorDetails } from './errorLog.js';
...
/**
 * Map AppError types to ErrorLog types
 */
function mapErrorTypeToErrorLog(appErrorType) {
  switch (appErrorType) {
    case ErrorTypes.VALIDATION_ERROR:
      return ErrorLogTypes.VALIDATION_ERROR;
    case ErrorTypes.AUTHENTICATION_ERROR:
    case ErrorTypes.AUTHORIZATION_ERROR:
      return ErrorLogTypes.AUTH_ERROR;
    case ErrorTypes.DATABASE_ERROR:
      return ErrorLogTypes.DATABASE_ERROR;
    case ErrorTypes.RATE_LIMIT_ERROR:
      return ErrorLogTypes.API_ERROR;
    case ErrorTypes.NOT_FOUND_ERROR:
    case ErrorTypes.BAD_REQUEST:
      return ErrorLogTypes.API_ERROR;
    case ErrorTypes.INTERNAL_ERROR:
    default:
      return ErrorLogTypes.UNHANDLED_EXCEPTION;
  }
}
...
export async function handleAPIError(error, req, res, userId = null) {
  ...
  if (error.isOperational) {
    ...
    if (error.type !== ErrorTypes.VALIDATION_ERROR) {
      ...
      await logError(
        mapErrorTypeToErrorLog(error.type),
        error.message,
        ...
        error.statusCode >= 500 ? ErrorSeverity.HIGH : ErrorSeverity.MEDIUM,
        userId,
        req.url,
        clientIP
      );
  ...
  await logError(
    ErrorLogTypes.UNHANDLED_EXCEPTION,
