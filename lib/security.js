 ...
 * CSRF protection, security logging, and monitoring
 */
...
export function validateCSRFToken(token, sessionId) {
  ...
  if (Date.now() - data.createdAt > 3600000) { // 1 hour expiry
    csrfTokens.delete(token);
    return false;
  }
  return true;
...
export function detectSuspiciousActivity(req) {
  ...
  if (referer && !referer.includes(process.env.NEXT_PUBLIC_SUPABASE_URL || '')) {
    // Referer from different domain (possible CSRF)
