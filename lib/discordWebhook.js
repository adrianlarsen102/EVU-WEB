...
export async function sendDiscordNotification(eventType, data, webhookUrl = null) {
  try {
    ...
    if (!isValidDiscordWebhookUrl(url)) {
      console.error('[Discord] Invalid webhook URL, skipping notification');
      return false;
    }
...
export async function testDiscordWebhook(webhookUrl) {
  ...
  if (!isValidDiscordWebhookUrl(webhookUrl)) {
    return { success: false, error: 'Invalid Discord webhook URL. Must be a valid https://discord.com/api/webhooks/ URL' };
  }
