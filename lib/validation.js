...
export function sanitizeString(input) {
  ...
  const filter = new xss.FilterXSS({
    ...
    stripIgnoreTagBody: ['script', 'style'] // remove the text content of these tags
  });

  return filter.process(input).trim().slice(0, 1000);
}
...
export function sanitizeHTML(input) {
  ...
  const filter = new xss.FilterXSS({
    whiteList: {
      b: [], i: [], em: [], strong: [], u: [], p: [], br: [], ul: [], ol: [], li: [], code: [], pre: [],
      a: ['href', 'target']
    },
    stripIgnoreTag: true,
  ...
  return filter.process(input).slice(0, 5000);
}
