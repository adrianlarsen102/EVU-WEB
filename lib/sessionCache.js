...
class SessionCache {
   ...
   * Called when user's role or permissions change
   * @param {string} userId - User ID (adminId)
   */
  invalidateUserSessions(userId) {
    let invalidatedCount = 0;
    for (const [sessionId, entry] of this.cache.entries()) {
      if (entry.session && entry.session.adminId === userId) {
        this.cache.delete(sessionId);
        invalidatedCount++;
      }
    }
...
// Export both as default and named for better compatibility
