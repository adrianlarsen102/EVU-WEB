 ...
 * Integrates with Discord webhook notifications
 */
...
export async function auditLog(
  ...
  try {
    ...
    try {
      ...
      // Send Discord notification (non-blocking, best-effort)
      try {
        // Only send for certain severity levels or event types
        const shouldNotify = severity === AuditSeverity.WARNING ||
        ...
        if (shouldNotify) {
          ...
          sendDiscordNotification(eventType, {
            ...
            // Silently log Discord errors
            console.log('[Discord] Notification failed:', err.message);
          });
        }
      } catch (discordError) {
        // Discord notification failed - this is non-critical, continue
        console.log('[Discord] Notification error (non-critical):', discordError.message);
      }
