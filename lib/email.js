...
import { getEmailSettings } from './database';
...
async function sendEmail(to, subject, htmlContent, textContent) {
  ...
  try {
    if (config.provider === 'smtp') {
      // SECURITY: Decrypt SMTP password if encrypted
      const decryptedSmtpPass = config.smtp_pass && isEncrypted(config.smtp_pass)
        ? decrypt(config.smtp_pass)
        : config.smtp_pass;
      ...
      // SECURITY: Decrypt Resend API key if encrypted
      const decryptedApiKey = config.resend_api_key && isEncrypted(config.resend_api_key)
        ? decrypt(config.resend_api_key)
